...
namespace WFuzzGen
{
    public class TestLibrary_Calculator_generator : WFuzz.ICaller
    {
        public object Call(WFuzz.FuzzInput input)
        {
            try
            {
                var type = typeof(TestLibrary.Calculator);
                ...
                throw new InvalidOperationException("无法创建类型 TestLibrary.Calculator 的实例");
            }
            catch (Exception ex)
            {
