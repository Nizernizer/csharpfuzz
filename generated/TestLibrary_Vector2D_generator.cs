...
namespace WFuzzGen
{
    public class TestLibrary_Vector2D_generator : WFuzz.ICaller
    {
        public object Call(WFuzz.FuzzInput input)
        {
            try
            {
                var type = typeof(TestLibrary.Vector2D);
                ...
                throw new InvalidOperationException("无法创建类型 TestLibrary.Vector2D 的实例");
            }
            catch (Exception ex)
            {
